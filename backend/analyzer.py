...
# 🪦 09/09/2026 — `analyze_all_sheets` FOI APAGADA AQUI, e é bom que se
# saiba por quê. Ela era o LADO MORTO de uma cópia dupla: o `main.py` a
# IMPORTAVA e NUNCA a chamava, e nela moravam consertos que a produção
# nunca recebeu. Três defeitos de 09/09 nasceram daí:
#   · o marcador de página no `ref_sheet` — 0 de 12.818 itens em 4 meses
#   · `confirmado + 0` inventando `qty = 1` com selo de MEDIDO — 14 dias
#   · o conselho de reprocessar, aposentado em 24/08 e ainda saindo
# Em todos, o guarda ficou VERDE porque testava ESTA função.
# 🔑 O que ela fazia de útil já vive no laço de item do `main.py`, e as
# decisões que dela sobraram viraram régua CHAMADA pelos dois lados:
# `sanear_qtd_e_selo` e `monta_ref_sheet`, ambas neste arquivo.
# 🚫 Não ressuscitar: guarda `test_main_nao_importa_o_que_nao_chama`.
