...
12. **RLS Supabase — auditoria COMPLETA e APLICADA em 2026-07-13** (a de 2026-06-02 nunca tinha sido aplicada). Fechados sem quebrar nada: `user_credits`, `chat_leads`, `agent_conversations`, catálogos, densidade, `projects` (INSERT/UPDATE), cotações, cronograma, notas/revisões, `project_items`, cashback, logos, Storage (pranchas/planilhas/anexos), e as RPCs SECURITY DEFINER (revogadas de anon/authenticated/PUBLIC, mantendo service_role). Relatório: `AUDITORIA_SEGURANCA_2026-07-13.md` (na pasta `arq/`, não commitado). ⚠️ **NUNCA revogar anon de `get_project_owner`** (o controle de dono `_get_project_owner` chama com a anon key — quebra download/itens/cronograma) nem de `search_sinapi/search_tcpo/get_tcpo_details` (matchers usam anon). `admin_list_all_signups` tem trava interna de admin (safe). Tabela "RLS on + 0 policies" = só service_role = correto. Pendente 1-clique do Pedro: ligar leaked-password protection no painel Auth.
